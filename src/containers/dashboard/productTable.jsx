...
import { setFewerProductsService, setToggleNotificationService } from "../../services/product";
...
const ProductTable = () => {
  ...
  const handleGetFewerProducts = async ()=>{
    setLoading(true)
    const res = await setFewerProductsService();
    setLoading(false)
    if (res.status == 200) {
  ...
  const handleTurnoffNotification = async (productId)=>{
    const res = await setToggleNotificationService(productId);
    if (res.status == 200) {
